...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    scrapers::scraper::init_webdriver();
    let questions = scrapers::scraper::scrape_grind75().await?;
    let mut filled_questions = Vec::new();

    for mut question in questions {
        let q = scrapers::scraper::get_problem_details(question).await?;
        filled_questions.push(q);
    }
    ...
    db_api::add_leetcode_entries_to_db(filled_questions, pool.as_ref()).await?;
